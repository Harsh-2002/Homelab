...
All three Proxmox nodes use AdGuard Home as the primary resolver and Cloudflare as the availability fallback. DNS is configured per node in `/etc/resolv.conf`; `systemd-resolved` is inactive.
