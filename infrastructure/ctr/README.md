...
VM 204 `ctr` mounts Store's encrypted SMB3 share `//smb.l3b.cc.cd/AV` at `/mnt/AV`. This is the external Crucial SSD's `external/share` volume on `store`, not `ctr`'s local 500 GB `/data` disk. The share has `downloads` and `media` directories for a future JDownloader and Jellyfin deployment; neither workload was deployed as part of this mount setup. Suggested container bind paths are `/mnt/AV/downloads` and `/mnt/AV/media`, while application configuration and databases stay under `/data/apps`.
