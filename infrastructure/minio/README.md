...
The Portainer `minio` Stack (ID `111`), Caddy routes, local working directory `/data/minio`, container image, and MinIO-specific runtime secrets were removed only after validation. The former recovery source at `/EX/RECOVERY/2026-09-15/SSD/Minio` was erased when the external SSD was reformatted on 2026-09-23. RustFS now needs its own independent backup.
