...
- The 4 TB Crucial X9 Pro was reformatted on 2026-09-23; the former whole-system tar and recovery tree were erased at the owner's request. Its two existing ext4 LVs are mounted on px10 and bound into unprivileged Debian CT 109 `store` (`10.1.1.12`): ~1.64 TiB `AV` and 2 TiB `PX`. The VG has no free extents. CT 109 has 1 vCPU, 1 GiB RAM, and an 8 GiB local-zfs OS disk; that OS disk replicates to px20 every five minutes, with HA restricted to px10/px20. The external USB data does **not** replicate: move the SSD physically to px20 and mount both LVs there before restarting Samba after px10 failure. See `infrastructure/store/README.md`.
- Samba 4.22.11 exposes two separate SMB3-encrypted shares with the same `Store SMB` vault credential: `smb://smb.l3b.cc.cd/AV` for general files and `smb://smb.l3b.cc.cd/PX` for native Proxmox backups. Both are direct LAN SMB, not HTTPS/Caddy. Their data resides on different ext4 LVs. The service refuses to start if its disk markers are absent. Beszel agent and Node Exporter run in CT 109.
