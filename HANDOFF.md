...
- PBS 4.2 and Samba run in `store` (`10.1.1.12`). The PBS UI is private at `pbs.l3b.cc.cd`; SMB is `smb://store.l3b.cc.cd/AV`. VM 107 replicates its OS disk to px20 and has HA affinity only to px10/px20. The USB data does not replicate, so physical disk movement is required after host failure.
