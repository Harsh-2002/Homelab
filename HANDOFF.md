...
- VM 204 `ctr` mounts the encrypted SMB3 share persistently at `/mnt/AV` using a systemd automount; `/mnt/AV/downloads` and `/mnt/AV/media` are prepared for future JDownloader/Jellyfin. The root-only CIFS credential comes from the existing `Store SMB` vault item. Application databases/config stay on local `/data/apps`. See `infrastructure/ctr/README.md`.
